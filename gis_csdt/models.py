...
class Field(models.Model):
    useName = models.CharField(max_length=150, default='name')
...
class Dataset(models.Model):
    ...
    cache_max_age = models.IntegerField(
        'age when cache should be replaced in days', default=1)
    # field names
    fields = models.ManyToManyField(Field, through='FieldOrder')
    ...
    def loop_thru_cache(self):
        ...
        points = MapPoint.objects.filter(
            dataset_id=self.pk).order_by('remote_id')
            ...
            for item in json_in:
                if i < len(points) and self.remote_id_field != '':
                    if points[i].remote_id == str(item[self.remote_id_field]):
                        ...
                        print points[i].remote_id, str(
                            item[self.remote_id_field])
                        try:
                ...
                for field in fields:
                    ...
                    if field in ['lat', 'lon']:
                        ...
                        temp = temp[0:MapPoint._meta.get_field(
                            field).max_length]
                ...
                try:
                    new_point.point = Point(
                        float(new_point.lon), float(new_point.lat))
                    new_point.save()
                except:
                    pass
                added += 1
                if added >= BATCH_SIZE:
                    if try_geocoding:
                        ...
                        print '--%d added--' % (added)
                    return
            json_in = json.loads(urllib.urlopen(
                self.url + plus + '$offset=' + str(rec_read)).read())
            rec_read += len(json_in)
...
class FieldOrder(models.Model):
    ...
    def __unicode__(self):
        return "#" + str(self.rank) + ". " + self.field.useName + "= \"" + self.field.setName + "\""
...
class MapPoint(MapElement):
    ...
    def geocode(self, unknown_count=0):
        ...
        request = 'https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=%s&sensor=false' % (
            location, key)
        #print request
