...
RECOGNIZED_PARAMETERS = ['max_lat', 'min_lat', 'max_lon', 'min_lon',
                         'dataset', 'radius', 'remote_id' 'tags', 'tag', 'state', 'zipcode']
...
def filter_request(parameters, model_type):
    ...
    if tags:
        ...
        if matchall:
            ...
            for tag in tags:
                try:
                    num = int(tag)
                    queryset = queryset | MapElement.objects.filter(
                        tagindiv__tag=num)
                except:
                    queryset = queryset | MapElement.objects.filter(
    ...
    queryset = square_filter(queryset, parameters, model_type)
    ...
    if 'radius' in parameters and 'center' in parameters:
        ...
        queryset = queryset.filter(
            point__distance_lte=(center, Distance(mi=radius)))
...
def neighboring_points(point, queryset, distance):
    distance = distance * 2
    all_points = queryset.filter(
        point__distance_lte=(point.point, distance)).distinct()
    point_set = list(all_points)  # .values_list('point',flat=True))
    for p in point_set:
        new_points = queryset.filter(point__distance_lte=(p.point, distance)).exclude(
            id__in=[x.id for x in point_set]).distinct()
...
def unite_radius_bubbles(points, distances):
    geometry = {}
    for d in distances:
        geometry[d] = circle_as_polygon(
            lat=points[0].point.y, lon=points[0].point.x, distance=d)
        for p in points[1:]:
            geometry[d] = geometry[d].union(circle_as_polygon(
                lat=p.point.y, lon=p.point.x, distance=d))
    return geometry


LOCATION_PARAMETERS = ['street', 'city', 'state', 'county', 'zipcode']
def location_filter(queryset, parameters):

    for key in LOCATION_PARAMETERS:
        if key in parameters:
            try:
                queryset = queryset.filter(
                    mappoint__dataset__fields_set__usename=parameters[key])
            except:
                pass
    if 'street' in parameters:
        queryset = queryset.filter(
            mappoint__dataset__fields_set __street__iexact=parameters['street'])
    if 'city' in parameters:
        queryset = queryset.filter(
            mappoint__city__iexact=parameters['city'])
    if 'state' in parameters:
        queryset = queryset.filter(
            mappoint__state__iexact=parameters['state'])
    if 'county' in parameters:
        queryset = queryset.filter(
            mappoint__county__iexact=parameters['county'])
    for key in ['zipcode', 'zip', 'zip_code']:
        if key in parameters:
            queryset = queryset.filter(
                mappoint__zipcode__iexact=parameters[key])


def square_filter(queryset, parameters, model_type):
    for key in ['max_lat', 'min_lat', 'max_lon', 'min_lon']:
        if key in parameters:
            try:
                ...
                raise ParseError(
                    'Invalid bounding box. Only floats accepted.')
                continue
            bb[key] = r
    if 'max_lat' in bb and 'min_lat' in bb and 'max_lon' in bb and 'min_lon' in bb:
        ...
        if model_type == 'mappoint':
            ...
            queryset = queryset.filter((~Q(mappoint=None) & Q(point__within=geom)) | (
                ~Q(mappolygon=None) & Q(mappolygon__mpoly__bboverlaps=geom)))
    return queryset


def dataset_filter(queryset, parameters):
    if 'dataset' in parameters:
        dataset_list = parameters['dataset'].strip().split(',')
        if type(dataset_list) is not list:
            dataset_list = [dataset_list]
...
def radius_filter(queryset, parameters):
    if 'radius' in parameters and 'center' in parameters:
        try:
            radius = int(parameters['radius'])
        except:
            raise ParseError(
                'Invalid radius. Only integers accepted.')
        temp = parameters.split(',')
        try:
            if len(temp) != 2:
                raise
            ...
            raise ParseError(
                'Invalid center. Format is: center=lon,lat')
        queryset = queryset.filter(
            point__point__distance_lte=(center, Distance(mi=radius)))
    elif 'radius' in parameters or 'center' in parameters:
        raise ParseError(
            'If a center or radius is specified, the other must also be specified.')
    return queryset
